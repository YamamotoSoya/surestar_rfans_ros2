...
inline int processPacketUser(PACKET_USER_S* packet, sensor_msgs::msg::PointCloud2 &outCloud)
{
    ...
    for(int i =0; i < GROUP_NUM_USER; i++)//10 bolck
    {
        ...
        int mirrorID = (mtBlock->flag&0x0F00 )>>10;
//        if(i <=8)
//        {
//            int tmpAngleDiff = packet->groups[i+1].azimuth_angle - packet->groups[i].azimuth_angle;
//            if (tmpAngleDiff < -35000)
...
//        }
        ...
        float angle_offset = (tmpAngleDiff*UINTCONVERT)/32.0;
        if(DEVICE_ID==0x4F||DEVICE_ID==0x55||DEVICE_ID==0x56||DEVICE_ID==0x57||DEVICE_ID==0x58)
        {
            angular_z = tmpAngleDiff*UINTCONVERT/(32*3.125);
        }
        ...
        for(int j=0; j< POINT_NUM_USER*2; j+=2)//16
        {
            ...
            case CFNAS_32_0x81:
                timeOffset = 6.25;
                tmpXyz1.timeflag = packet->gps_timestamp + timeOffset*(j+i*POINT_NUM_USER*2);
                tmpXyz2.timeflag = packet->gps_timestamp + timeOffset*(j+1+i*POINT_NUM_USER*2);
                tmpAngle1 = mtBlock->azimuth_angle*UINTCONVERT + j*angle_offset;
                tmpAngle2 = mtBlock->azimuth_angle*UINTCONVERT + (j+1)*angle_offset;
                break;
            default:
                break;
            }
            ...
            tmpXyz1.laserid = j;
            tmpXyz2.laserid = j+1;
            if(CFNAS_32_0x81 == DEVICE_ID) {
                calcCFansXYZ_32(tmpRange1, tmpAngle1, mirrorID,  tmpXyz1);
                calcCFansXYZ_32(tmpRange2, tmpAngle2, mirrorID,  tmpXyz2);
            } else {
...
//        }
        ...
        float angle_offset = (tmpAngleDiff*UINTCONVERT)/32;
        if(DEVICE_ID==0x4F||DEVICE_ID==0x55||DEVICE_ID==0x56||DEVICE_ID==0x57||DEVICE_ID==0x58){
            angular_z = tmpAngleDiff*UINTCONVERT/(32*3.125);
        }
        else if(DEVICE_ID==0x5C ||DEVICE_ID==0x5B){
        ...
        for(int j=0; j< POINT_NUM_USER_SIMPLE; j++)
        {
            ...
            case CFNAS_32_0x81:
                timeOffset = 6.25;
                tmpAngle = mtBlock->azimuth_angle*UINTCONVERT + j*angle_offset;
                tmpXyz.timeflag = packet->gps_timestamp +
                        timeOffset * ((j/echo_flag) + i * POINT_NUM_USER_SIMPLE);
                break;
            ...
            tmpRange = mtBlock->points[j].range*CONVERT_4mm_2m;
            tmpXyz.mirrorid =0;
            if(DEVICE_ID == CFNAS_32_0x81){
                //                int ringID = i%8;
                calcCFansXYZ_32(tmpRange,tmpAngle,mirrorID,tmpXyz);
            }else {
                calcXyz(DEVICE_ID,tmpRange,tmpAngle,tmpXyz);
            }
            if(g_xyzFile) {
            ...
            if(DEVICE_ID == CFNAS_32_0x81) {
                ...
                tmpXyz.laserid = 4*tmpXyz.laserid + temp;
            }
            s_lineData[s_lineCount] = tmpXyz;
            ++s_lineCount;
            if(s_lineCount>=LINE_POINT_COUNT) s_lineCount = LINE_POINT_COUNT-1;
...
//        }
        ...
        float angle_offset = (tmpAngleDif*UINTCONVERT)/32;
        if(mtFrame->gmReservedA==0x4F||mtFrame->gmReservedA==0x55||mtFrame->gmReservedA==0x56||mtFrame->gmReservedA==0x57||mtFrame->gmReservedA==0x58)
        {
            angular_z= tmpAngleDif*UINTCONVERT/(32*3.125);//deg/us
        }
        ...
        for (int i = 0; i < 32; i++) {
            ...
            case CFNAS_32_0x81:
                timeOffset = 6.25;
                tmpAngle = mtBlock->azimuthAngle*UINTCONVERT + i*angle_offset;
                tmpXyz.timeflag = mtFrame->gpsTimestamp +
                        timeOffset * (i + j * POINT_NUM_USER_SIMPLE);
                break;
            default:
                break;
            }
